...
export const BlogsView: React.FC<BlogsViewProps> = ({ setTab, setSelectedBlogId }) => {
  ...
  const categories = useMemo(() => {
    const unique = Array.from(new Set(blogs.map((post) => post.category || 'Uncategorized')));
    return ['All', ...unique];
  }, [blogs]);

  const filteredBlogs = blogs.filter((post) => {
    const searchMatch = [post.title, post.excerpt, post.author, post.tags?.join(' ') || '']
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 md:p-8">
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {isLoading ? (
              ...
              <div className="col-span-full rounded-3xl border border-slate-200 bg-slate-50 p-8 text-center text-brand-red">
                {error}
              </div>
              ...
              filteredBlogs.map((post: BlogPost) => {
                const postKey = post.slug || post.id || `${post.title}-${post.publishedAt}`;
                const selectedId = post.slug || post.id || postKey;
                return (
                  <article
                    key={postKey}
                    onClick={() => {
                    ...
                    className="group cursor-pointer rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
                  >
